            ...
            case MO_CHI_SO_BONG_TAI_CAP3:
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    if (bongTai != null && gallery != null && daXanhLam != null && gallery.quantity >= 99) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "1Cần 1 Bông tai Porata cấp 3, X99 Đá Gellery và 1 Đá xanh lam", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "2Cần 1 Bông tai Porata cấp 3, X99 Đá Gellery và 1 Đá xanh lam", "Đóng");
                }
                break; 
...
//    }
    ...
    private void nhapNgocRong(Player player) {
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            if (!player.combineNew.itemsCombine.isEmpty()) {
                Item item = player.combineNew.itemsCombine.get(0);
                if (item != null && item.isNotNullItem() && (item.template.id > 14 && item.template.id <= 20) && item.quantity >= 4) {
...
//                        }
   ...
   private boolean isDaPhaLe(Item item) {
        return item != null && (item.template.type == 30 || item.template.type == 32 || (item.template.id >= 14 && item.template.id <= 20));
    }
